-- ============================================================
-- Billy's Fast Food - Migration 004 v2: New Menu Structure
-- Adds new categories and products based on menu images
-- ============================================================

-- ── Delete existing categories 5-9 if they exist ────────────
DELETE FROM product_options WHERE product_id IN (SELECT id FROM products WHERE category_id IN (5,6,7,8,9));
DELETE FROM product_restaurant WHERE product_id IN (SELECT id FROM products WHERE category_id IN (5,6,7,8,9));
DELETE FROM products WHERE category_id IN (5,6,7,8,9);
DELETE FROM categories WHERE id IN (5,6,7,8,9);
...
-- ── Update existing Burgers category ────────────────────────
UPDATE `categories` SET `sort_order` = 10 WHERE `id` = 1;
UPDATE `categories` SET `name` = 'Frites', `slug` = 'frites', `description` = 'Nos frites', `sort_order` = 41 WHERE `id` = 2;
...
-- ── Add all new products to restaurant 1 ────────────────────
